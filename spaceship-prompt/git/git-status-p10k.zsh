...
spaceship_git_status_p10k() {
  ...
  if [[ -n $VCS_STATUS_LOCAL_BRANCH ]]; then
    ...
    git_status+="${SPACESHIP_GIT_P10K_CLEAN_COLOR}${(g::)SPACESHIP_GIT_P10K_BRANCH_SYMBOL}${branch//\%/%%}"
  fi
  
  [[ -n ${VCS_STATUS_REMOTE_BRANCH:#$VCS_STATUS_LOCAL_BRANCH} ]] &&
    git_status+="${SPACESHIP_GIT_P10K_CLEAN_COLOR}${(g::)SPACESHIP_GIT_P10K_REMOTE_BRANCH_SYMBOL}${(V)VCS_STATUS_REMOTE_BRANCH//\%/%%}"

  [[ -z $VCS_STATUS_LOCAL_BRANCH && -z $VCS_STATUS_TAG ]] &&
    git_status+="${SPACESHIP_GIT_P10K_CLEAN_COLOR}${(g::)SPACESHIP_GIT_P10K_COMMIT_SYMBOL}${VCS_STATUS_COMMIT[1,8]}"

  if [[ -n $VCS_STATUS_TAG && -z $VCS_STATUS_LOCAL_BRANCH ]]; then
    local tag=${(V)VCS_STATUS_TAG}
    (( $#tag > 32 )) && tag[13,-13]="…"
    git_status+="${SPACESHIP_GIT_P10K_CLEAN_COLOR}${(g::)SPACESHIP_GIT_P10K_TAG_SYMBOL}${tag//\%/%%}"
  fi
  ...
  if (( VCS_STATUS_COMMITS_AHEAD || VCS_STATUS_COMMITS_BEHIND )); then
    ...
    (( VCS_STATUS_COMMITS_BEHIND )) && git_status+=" ${SPACESHIP_GIT_P10K_CLEAN_COLOR}${(g::)SPACESHIP_GIT_P10K_COMMITS_BEHIND_SYMBOL}${VCS_STATUS_COMMITS_BEHIND}"
    # ⇡42 if ahead of the remote; no leading space if also behind the remote: ⇣42⇡42.
    (( VCS_STATUS_COMMITS_AHEAD && !VCS_STATUS_COMMITS_BEHIND )) && git_status+=" "
    (( VCS_STATUS_COMMITS_AHEAD  )) && git_status+="${SPACESHIP_GIT_P10K_CLEAN_COLOR}${(g::)SPACESHIP_GIT_P10K_COMMITS_AHEAD_SYMBOL}${VCS_STATUS_COMMITS_AHEAD}"
  fi

  # ⇠42 if behind the push remote.
  (( VCS_STATUS_PUSH_COMMITS_BEHIND )) && git_status+=" ${SPACESHIP_GIT_P10K_CLEAN_COLOR}${(g::)SPACESHIP_GIT_P10K_PUSH_COMMITS_BEHIND_SYMBOL}${VCS_STATUS_PUSH_COMMITS_BEHIND}"
  (( VCS_STATUS_PUSH_COMMITS_AHEAD && !VCS_STATUS_PUSH_COMMITS_BEHIND )) && git_status+=" "
  # ⇢42 if ahead of the push remote; no leading space if also behind: ⇠42⇢42.
  (( VCS_STATUS_PUSH_COMMITS_AHEAD  )) && git_status+="${SPACESHIP_GIT_P10K_CLEAN_COLOR}${(g::)SPACESHIP_GIT_P10K_PUSH_COMMITS_AHEAD_SYMBOL}${VCS_STATUS_PUSH_COMMITS_AHEAD}"
  # *42 if have stashes.
  (( VCS_STATUS_STASHES        )) && git_status+=" ${SPACESHIP_GIT_P10K_CLEAN_COLOR}${(g::)SPACESHIP_GIT_P10K_STASHES_SYMBOL}${VCS_STATUS_STASHES}"
  # 'merge' if the repo is in an unusual state.
  [[ -n $VCS_STATUS_ACTION     ]] && git_status+=" ${SPACESHIP_GIT_P10K_CONFLICTED_COLOR}${VCS_STATUS_ACTION}"
  # ~42 if have merge conflicts.
  (( VCS_STATUS_NUM_CONFLICTED )) && git_status+=" ${SPACESHIP_GIT_P10K_CONFLICTED_COLOR}${(g::)SPACESHIP_GIT_P10K_CONFLICTED_SYMBOL}${VCS_STATUS_NUM_CONFLICTED}"
  ...
  (( VCS_STATUS_NUM_UNSTAGED   )) && git_status+=" ${SPACESHIP_GIT_P10K_MODIFIED_COLOR}${(g::)SPACESHIP_GIT_P10K_UNSTAGED_SYMBOL}${VCS_STATUS_NUM_UNSTAGED}"
  # ?42 if have untracked files. It's really a question mark, your font isn't broken.
  (( VCS_STATUS_NUM_UNTRACKED  )) && git_status+=" ${SPACESHIP_GIT_P10K_UNTRACKED_COLOR}${(g::)SPACESHIP_GIT_P10K_UNTRACKED_SYMBOL}${VCS_STATUS_NUM_UNTRACKED}"
